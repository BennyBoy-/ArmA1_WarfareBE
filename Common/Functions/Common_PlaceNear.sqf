...
//Place object within random radius of position.
//[position,minRadius,maxRadius,faceAway,randomDir] Call PlaceNear
//object - object to be placed.
//position - center of random placement.
//minRadius - min distance from center.
//maxRadius - max distance from center.
//faceAway - if true set object's direction away from the center.
//randomDirection - if true set object to random direction.  faceAway overrides this.
...
if (_placeSafe) then
{
	...
	_destination = [(_position Select 0)+((sin _direction)*_radius),(_position Select 1)+((cos _direction)*_radius),_position Select 2];
	[_object,_destination,_safeRadius] Call PlaceSafe;
}
else
{
	_object SetPos [(_position Select 0)+((sin _direction)*_radius),(_position Select 1)+((cos _direction)*_radius),(_position Select 2)];
};

if (_randomDirection) then
{
